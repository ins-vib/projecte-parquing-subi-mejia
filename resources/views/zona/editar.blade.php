    ...
    <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto lg:py-0">
        <div style="margin: 1%" class="w-full bg-white rounded-lg shadow dark:border sm:max-w-2xl xl:p-0 dark:bg-gray-800 dark:border-gray-700">
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form method="POST" action="{{ route('zona.editar', $zona->id) }}" class="space-y-4">
                    @csrf
                    @method('POST')

                    <div>
                        <label for="nom" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nom de la Zona:</label>
                        <input type="text" name="nom" id="nom" value="{{ old('nom', $zona->nom) }}" required
                    ...
                    <div>
                        ...
                        <select name="parking_id" id="parking_id" required
                            ...
                            @foreach($parkings as $parking)
                                <option value="{{ $parking->id }}" {{ old('parking_id', $zona->parking_id) == $parking->id ? 'selected' : '' }}>
                                    {{ $parking->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="capacitatTotal" id="capacitatTotal" value="{{ old('capacitatTotal', $zona->capacitatTotal) }}" min="1" required
                            class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white" />
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        @foreach($tipusPlaces as $tipus)
                            @php
                                $quantitat = $placesPerTipus->has($tipus->id) ? $placesPerTipus[$tipus->id]->total : 0;
                            @endphp
                            <div>
                                <label class="block mb-1 text-sm font-medium text-gray-900 dark:text-white">
                                    {{ $tipus->nom }}
                                </label>
                                <input type="hidden" name="tipus_vehicle[]" value="{{ $tipus->id }}">
                                <input 
                                    type="number" 
                                    name="quantitat[]" 
                                    min="0"
                                    value="{{ old('quantitat.' . $loop->index, $quantitat) }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                                    placeholder="Quantitat per {{ $tipus->nom }}"
                                >
                            </div>
                        @endforeach
                    </div>
